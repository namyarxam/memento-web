...
const SOURCE_ICONS = {
  'claude.ai': (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" className="source-icon">
      <path d="M16.98 11.39l-5.505-8.672a1.4 1.4 0 0 0-1.088-.568 1.376 1.376 0 0 0-1.332.9l-5.677 14.66a1.376 1.376 0 0 0 .108 1.2c.25.387.672.63 1.132.642l6.44.168a.68.68 0 0 0 .528-.24.72.72 0 0 0-.012-.96.68.68 0 0 0-.468-.228l-6.1-.156 5.4-13.944 5.232 8.232a.69.69 0 0 0 .576.312.69.69 0 0 0 .588-.324.72.72 0 0 0 .012-.744l-.144-.228-.096-.048zm2.1 5.16l-5.1-8.028a.69.69 0 0 0-.576-.312.69.69 0 0 0-.588.324.72.72 0 0 0-.012.744l4.86 7.656-12.54-.324 3.06-7.884a.72.72 0 0 0-.06-.636.69.69 0 0 0-.528-.324.68.68 0 0 0-.576.192.72.72 0 0 0-.18.384l-3.36 8.664a1.38 1.38 0 0 0-.108 1.2c.25.384.672.624 1.128.636l13.14.348a1.38 1.38 0 0 0 1.2-.564 1.38 1.38 0 0 0 .24-1.08z" fill="#D97757"/>
    </svg>
  ),
  'chatgpt.com': (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" className="source-icon">
    ...
    <div className="dashboard">
      ...
      {activeBox ? (
        <div className="box-view-header">
          <button className="box-view-back" onClick={() => setActiveView('all')}>
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              <path d="M8.75 3.5L5.25 7L8.75 10.5" stroke="currentColor" strokeWidth="1.5"
                strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </button>
        ...
        <div className="dashboard-header">
          <h2>
            Mementos
            <span className="dashboard-count">{captures.length}</span>
          </h2>
        </div>
      )}
                  ...
                  {isEditing ? (
                    <div className="memento-tagline-edit" onClick={e => e.stopPropagation()}>
                      <input
                        ...
                        autoFocus
                      />
                      <button className="btn btn-sm btn-primary" onClick={() => saveTagline(c.id, editingTaglineVal)}>Save</button>
                      <button className="btn btn-sm" onClick={() => setEditingTaglineId(null)}>Cancel</button>
                    </div>
                  ) : (
                    <>
                      <span
                      ...
                      {!c.tagline && !c._generating && (
                        <button
                          className="memento-generate-btn"
                      ...
                      {c._generating && (
                        <span className="memento-generating">...</span>
                      )}
                      {c.tagline && (
                        <button
                      ...
                      {blocks.map((block, i) => (
                        <div key={i} className={`memento-block memento-block-${block.role || 'unknown'}`}>
                          {block.role && block.role !== 'unknown' && (
                            <div className="memento-block-role">{block.role}</div>
                          )}
                      ...
                      <div className="memento-footer-actions">
                        {!c.tagline && (
                          <button className="btn btn-sm btn-ghost"
                            ...
                            onClick={e => { e.stopPropagation(); generateTagline(c.id) }}>
                            {c._generating ? 'Generating...' : '✨ Generate tagline'}
                          </button>
                        )}
                        {jumpHref && (
